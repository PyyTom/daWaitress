    ...
    def upd(how):
        ...
        if how == 'login':
            if d_user.value is None or t_password.value is None:alert.title = Text('EMPTY FIELDS')
                ...
                page.clean()
                page.add(home_view(page,theme,alert))
        elif how == 'register':
            if not t_user.value or not t_password.value or not t_re_password.value:alert.title = Text('EMPTY FIELDS')
                ...
                page.clean()
                page.add(login_view(page,theme,alert))
        elif how == 'unregister':
        ...
        page.update()
    db = sqlite3.connect('db.db')
    if db.execute('select * from USERS').fetchall()!=[]:d_user = Dropdown(label='USERNAME',width=200,options=[dropdown.Option(user[0]) for user in db.execute('SELECT USER FROM USERS').fetchall()])
    else:d_user = Dropdown(label='USERNAME', width=200)
    db.close()
